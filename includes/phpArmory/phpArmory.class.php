...
class phpArmory5 {
     ...
     */
    protected $armory = "http://eu.wowarmory.com/";
    ...
    protected function getXmlData($url, $userAgent = NULL, $timeOut = NULL) {
        ...
        $fileMD5 = md5(md5($url));
        if($cached = @file_get_contents('_cache/' . $fileMD5 .'.cache')) {
            return array ( 'result' => TRUE, 'XmlData' => $cached);
        }
        ...
        @file_put_contents('_cache/' . $fileMD5 . '.cache', $f);
    ...
    public function getGuildData($guildName = NULL, $realmName = NULL) {
        ...
        if (is_string($guildName) && is_string($realmName)) {
           // $guildName  = ucfirst($guildName);
            ...
            $armoryURL = $this->armory."guild-info.xml?r=" . urlencode($realmName) . "&gn=" . urlencode($guildName);
